...
const t = (key, fallback) => i18n.t(`usage.${key}`, fallback);
...
const render = (container, path, result) => {
    container.replaceChildren();
    appendText(container, 'p', path, 'fw-semibold text-break');
    appendText(container, 'p', t('scope', 'Shows direct editorial references. Templates, themes and dynamic queries are not included.'), 'text-body-secondary');
    if (result.problems.length) {
        const warning = appendText(container, 'div', t('incomplete', 'Some sources could not be fully checked. This list may be incomplete.'), 'alert alert-warning');
        warning.setAttribute('role', 'status');
        const details = document.createElement('details');
        appendText(details, 'summary', t('problems', 'Details'));
        const list = document.createElement('ul');
        result.problems.forEach(problem => appendText(list, 'li', `${problem.site} / ${problem.path}: ${problem.message}`, 'text-break'));
...
export const runAction = () => {
    ...
    openModal({
        ...
        onShow: async (modalElement) => {
            const container = modalElement.querySelector('[data-usage-content]');
            ...
            try {
                ...
                render(container, node.uri, response.result);
            }
            catch (error) {
                container.replaceChildren();
                const message = appendText(container, 'div', t('error', 'Could not load usages.'), 'alert alert-danger');
