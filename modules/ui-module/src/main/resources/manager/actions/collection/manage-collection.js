...
import { createCollectionItemEditor } from './edit-collection-item.js';
...
export const runAction = async (options) => {
    ...
    const closeEditor = () => {
        ...
        if (browsePanel)
            browsePanel.inert = false;
        if (editorPanel)
            editorPanel.inert = true;
    };
    const openEditor = async (id) => {
        if (!collectionSlider || !editorContent || !editorSaveButton)
            return;
        ...
        try {
            ...
            if (version !== editorRequestVersion)
                return;
    ...
    const saveEditor = async () => {
        if (!currentEditor || !editorSaveButton || isSaving || !currentEditor.form.validate())
            return;
        isSaving = true;
        editorSaveButton.disabled = true;
        const saved = await currentEditor.save();
        isSaving = false;
        if (!saved && currentEditor)
            editorSaveButton.disabled = false;
    };
    const update = async () => {
        const version = ++requestVersion;
        const root = modalElement?.querySelector('[data-collection-results]');
        const pagination = modalElement?.querySelector('[data-collection-pagination]');
        if (!root || !pagination)
            return;
