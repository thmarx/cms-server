...
import { searchPages } from '@cms/modules/rpc/rpc-page.js';
...
const renderResults = (results, selectText) => {
    if (results.length === 0) {
...
export const openPagePicker = (options) => {
    let modal;
    let searchVersion = 0;
    let debounceTimer;
    ...
    modal = openModal({
        ...
        showFooter: false,
        onShow: (modalElement) => {
            const input = modalElement.querySelector('#cms-page-picker-input');
            const resultsElement = modalElement.querySelector('[data-page-picker-results]');
            const showHint = () => {
                resultsElement.innerHTML = renderMessage(i18n.t('page.search.minLength', 'Enter at least 3 characters to search.'));
            };
            const runSearch = async () => {
                ...
                try {
                    ...
                    resultsElement.querySelectorAll('[data-page-picker-index]').forEach(button => {
