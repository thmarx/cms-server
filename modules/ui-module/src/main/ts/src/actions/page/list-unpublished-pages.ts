...
const renderPageListHtml = (pages: ItemDto[], currentPage: number, totalPages: number) => {
    let pageItemsHtml = '';
    if (pages.length === 0) {
    ...
    const disabledState = (pageNumber: number) => pageNumber < 1 || pageNumber > totalPages ? 'disabled' : '';
