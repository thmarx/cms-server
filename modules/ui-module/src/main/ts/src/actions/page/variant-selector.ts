...
const option = (selector: VariantSelectorDto, selected: string): string =>
	`<option value="${escapeHtml(selector.id)}"${selector.id === selected ? ' selected' : ''}>`
		+ `${escapeHtml(selector.label)}</option>`;

const showError = (error: unknown) => showToast({
	title: i18n.t(
		'manager.actions.page.variant-selector.error.title',
		'Could not configure variant selection'
	),
...
export const runAction = async () => {
	try {
		...
		openModal({
			title: i18n.t(
				'manager.actions.page.variant-selector.title',
			...
			fullscreen: false,
			onCancel: () => {},
			validate: () => Boolean((document.getElementById(SELECT_ID) as HTMLSelectElement)?.value),
			onOk: async () => {
				try {
					const selector = (document.getElementById(SELECT_ID) as HTMLSelectElement).value;
					await setVariantSelector(contentNode.result.uri, selector);
					showToast({
						title: i18n.t(
							'manager.actions.page.variant-selector.success.title',
							'Variant selection configured'
						),
						message: i18n.t(
							'manager.actions.page.variant-selector.success.message',
							'The selection strategy was saved.'
						),
						type: 'success',
						timeout: 3000
					});
