...
import { createUser, deleteUser, listRoles, listUsers, ManagerUser, Role, updateUser } from '@cms/modules/rpc/rpc-access.js';
...
class UserManager {
	...
	async show(): Promise<void> {
		...
		try { [this.users, this.roles] = await Promise.all([listUsers(), listRoles()]); this.render(); }
		catch (error) { this.fail('Could not load users', error); }
	}
	...
	private render(): void {
		...
		this.root.querySelectorAll<HTMLElement>('[data-edit]').forEach(button => button.addEventListener('click', () => {
			const username = (button.closest('[data-user]') as HTMLElement).dataset.user || '';
			this.editor(this.users.find(user => user.username === username)!, false);
		}));
		this.root.querySelectorAll<HTMLElement>('[data-delete]').forEach(button => button.addEventListener('click', async () => {
			const username = (button.closest('[data-user]') as HTMLElement).dataset.user || '';
			if (!confirm(`Delete user “${username}”?`)) return;
			try { await deleteUser(username); await this.show(); this.toast('User deleted', `${username} was deleted.`); }
			catch (error) { this.fail('Could not delete user', error); }
		}));
	}

	private editor(user: ManagerUser, isNew: boolean): void {
		const host = this.root.querySelector('[data-user-editor]') as HTMLElement;
		...
		host.querySelector('[data-save]')?.addEventListener('click', async () => {
			const username = (host.querySelector('[data-username]') as HTMLInputElement).value.trim();
			const mail = (host.querySelector('[data-mail]') as HTMLInputElement).value.trim();
			const password = (host.querySelector('[data-password]') as HTMLInputElement).value;
			const roles = Array.from(host.querySelectorAll<HTMLInputElement>('input[type=checkbox]:checked')).map(input => input.value);
			if (!username || (isNew && !password) || roles.length === 0) { this.toast('Incomplete user', 'Username, role and initial password are required.', 'error'); return; }
			try { isNew ? await createUser({ username, mail, roles }, password) : await updateUser({ username, mail, roles }, password);
				await this.show(); this.toast('User saved', `${username} was updated.`); }
			catch (error) { this.fail('Could not save user', error); }
		});
	}
