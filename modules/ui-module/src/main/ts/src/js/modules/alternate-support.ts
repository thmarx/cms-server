...
export const alternatesSupported = (content?: ActivePreviewContent | null): boolean =>
	Boolean(content?.uri) && content?.contentKind !== 'collection';

export const ensureAlternatesSupported = (content?: ActivePreviewContent | null): boolean => {
	if (alternatesSupported(content)) {
		return true;
	}
	showToast({
