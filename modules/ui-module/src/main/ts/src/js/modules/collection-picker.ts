...
import {
	...
	CollectionItemsPage,
	listCollectionItems
} from '@cms/modules/rpc/rpc-collection.js';
...
const renderResults = (response: CollectionItemsPage, selectText: string): string => {
...
export const openCollectionItemPicker = (options: CollectionItemPickerOptions): void => {
	let modal: any;
	...
	modal = openModal({
		...
		onShow: (modalElement: HTMLElement) => {
			const input = modalElement.querySelector(`#${inputId}`) as HTMLInputElement;
			const resultsElement = modalElement.querySelector('[data-collection-picker-results]') as HTMLElement;
			...
			const loadPage = async () => {
				...
				try {
					...
					resultsElement.querySelectorAll<HTMLElement>('[data-collection-picker-index]').forEach(button => {
					...
					resultsElement.querySelector<HTMLElement>('[data-collection-picker-previous]')
						?.addEventListener('click', () => {
							currentPage--;
							loadPage();
						});
					resultsElement.querySelector<HTMLElement>('[data-collection-picker-next]')
						?.addEventListener('click', () => {
							currentPage++;
							loadPage();
						});
