...
import com.github.thmarx.cms.content.markdown.BlockContainer;
import com.github.thmarx.cms.content.markdown.BlockElementRule;
import com.github.thmarx.cms.content.markdown.BlockRenderer;
...
public class BlockquoteBlockRule implements BlockElementRule {
	...
	public Block next(String md) {
		...
		if (matcher.find()) {
			...
			return new BlockquoteBlock(matcher.start(), matcher.end(), content);
		}
		return null;
	}

	public static record BlockquoteBlock(int start, int end, String content) implements Block, BlockContainer {
		...
		public String render(BlockRenderer blockRenderer) {
			return "<blockquote>%s</blockquote>".formatted(blockRenderer.render(content));
		}
